...
class DDRGame:
    ...
    def draw_menu(self):
        ...
        if hasattr(self, 'socket_thread') and self.socket_thread and self.socket_thread.is_alive():
            connection_text = self.small_font.render("Socket connected - Receiving keyboard inputs", True, GREEN)
        else:
            connection_text = self.small_font.render("Socket not connected - Waiting for keyboard publisher", True, RED)
        ...
        self.screen.blit(connection_text,
                         (SCREEN_WIDTH // 2 - connection_text.get_width() // 2, 350))
    ...
    def draw_game(self):
        ...
        input_text = self.small_font.render("Remote Keyboard Input: ACTIVE", True, GREEN)
    ...
    def draw_file_selector(self):
        ...
        self.screen.blit(connection_text,
                       (SCREEN_WIDTH // 2 - connection_text.get_width() // 2, 550))
