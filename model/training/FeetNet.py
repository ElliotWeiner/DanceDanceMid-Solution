...
class FeetNet(nn.Module):
    def __init__(self, num_views):
        ...
        self.backbone1_r2p1d_conv = nn.Sequential(*list(models.video.r2plus1d_18(pretrained=True, progress=True).children())[:-1])
        self.backbone2_r2p1d_conv = nn.Sequential(*list(models.video.r2plus1d_18(pretrained=True, progress=True).children())[:-1])
    ...
    def forward(self, v1, v3):#, v3, v4):
        ...
        pov2_maps = self.backbone2_r2p1d_conv(v3)
        ...
        combined = torch.concat((p1, p2), dim=1)
